// ADUGAME remaining commercial-art v1.03 polish.
// Keeps authored context visible without washing out the live Phaser interaction layer.
(() => {
  const CFG={
    G1R1:{wash:'linear-gradient(180deg,rgba(231,246,250,.15),rgba(249,250,246,.12) 60%,rgba(217,229,224,.15) 60.2% 100%)',hero:{x:1040,y:350,w:305},bgOpacity:'.10'},
    ...
    G2R3:{wash:'linear-gradient(180deg,rgba(235,242,247,.13),rgba(248,244,232,.10) 58%,rgba(194,177,151,.12) 58.2% 100%)',hero:{x:1045,y:345,w:235}}
  };
  ...
  function apply(scene){
    ...
    if(root.dataset.v103Polished==='1')return;
    root.dataset.v103Polished='1';root.dataset.version='1.03';root.dataset.interactionPriority='live-phaser-first';root.dataset.generatedVisualAssets='0';
    ...
    [hero,secondary].filter(Boolean).forEach(e=>{e.style.filter='saturate(1.04) contrast(1.025) drop-shadow(0 9px 14px rgba(48,56,82,.08))';});
    ...
    if(window.__ADUGAME_ART_SOURCE__?.REMAINING){window.__ADUGAME_ART_SOURCE__.REMAINING.version='commercial-v1.03';window.__ADUGAME_ART_SOURCE__.REMAINING.liveInteractionPriority=true;window.__ADUGAME_ART_SOURCE__.REMAINING.generatedVisualAssets=0;}
  }
  ...
  window.__ADUGAME_REMAINING_COMMERCIAL_ART_POLISH__={loaded:true,version:'1.03',liveInteractionPriority:true,generatedVisualAssets:0};
})();