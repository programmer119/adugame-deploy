// ADUGAME G1R2 v17.29 final dynamic UX + composition polish.
// Coordinates/UI over existing authored assets only. No generated/drawn illustration assets.
(() => {
  ...
  const FACE_FRAME={left:'54.6%',top:'33.6%',width:'78%',height:'90%'};
  const pct=(v,b)=>`${v/b*100}%`;

  function attach(scene){
    if(scene.scene?.key!=='G1R2'||scene.__g1v1729FinalUx)return;
    const root=document.getElementById('g1r2-v17-overlay');
    if(!root){scene.time.delayedCall(80,()=>attach(scene));return;}
    scene.__g1v1729FinalUx=true;
    ...
    const bathroom=root.querySelector('.g1v17-bathroom-context');
    ...
    Object.assign(alert.style,{position:'absolute',left:'50%',bottom:'4.2%',transform:'translateX(-50%)',zIndex:'22',pointerEvents:'none',padding:'10px 18px',borderRadius:'16px',border:'2px solid rgba(221,150,40,.48)',background:'rgba(255,246,222,.98)',boxShadow:'0 7px 18px rgba(96,64,16,.16)',fontSize:'15px',fontWeight:'1000',color:'#704812',opacity:'0',transition:'opacity .12s ease,transform .12s ease',whiteSpace:'nowrap'});
    ...
    const sync=()=>{
      ...
      // Keep the baby-face interaction target in place while revealing more of the authored helper character.
      if(st===2&&face){
        face.style.left=FACE_FRAME.left;face.style.top=FACE_FRAME.top;face.style.width=FACE_FRAME.width;face.style.height=FACE_FRAME.height;
        face.style.transform='translate(-50%,-50%)';face.style.filter='saturate(1.10) contrast(1.018)';
      }
      // The bathroom remains visible, but quieter during the close face-wash action.
      if(st===2&&bathroom)bathroom.style.opacity='.08';
      ...
      // On a miss, replace the stacked helper/progress/status messages with one persistent correction.
      if(st===0&&progress){progress.textContent=active==='paste'?'치약을 칫솔로 이동 중':'치약을 잡아 → 오른쪽 칫솔 솔에 놓기';progress.style.minWidth='230px';progress.style.opacity=miss?'0':'1';}
      if(st===0&&status){status.textContent='치약 튜브를 잡고 오른쪽 칫솔의 흰 솔 부분에 놓아 주세요';status.style.opacity=miss?'0':'1';}

      root.dataset.toolHitAlignmentReady='1';root.dataset.finalAlertReady='1';root.dataset.visualPolishReady='1';root.dataset.version='17.29';
      if(window.__ADUGAME_ART_SOURCE__?.G1R2){
        window.__ADUGAME_ART_SOURCE__.G1R2.version='v17.29';
        window.__ADUGAME_ART_SOURCE__.G1R2.dynamicUxFinal={pasteHome:{...PASTE_HOME},brushPasteTarget:{...BRUSH_PASTE_TARGET},brushHome:{...BRUSH_HOME},clothHome:{...CLOTH_HOME},visibleToolHitAlignment:true,pasteMissFeedback:true,persistentMissAlert:true,missPersistsUntilRetry:true,pasteDropTarget:true,pointerPositionGuard:true,finalDomPointerGuard:true,missSingleMessageMode:true,faceWashFraming:{...FACE_FRAME},faceWashBackdropOpacity:.08,generatedVisualAssets:0};
        window.__ADUGAME_ART_SOURCE__.G1R2.generatedVisualAssets=0;
      }
    };
    scene.events.on('postupdate',sync);sync();
    const cleanup=()=>{window.removeEventListener('pointerdown',track,true);window.removeEventListener('pointermove',track,true);dropTarget.remove();alert.remove();scene.__g1v1729FinalUx=false;};
