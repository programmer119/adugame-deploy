// ADUGAME G1R2 v17.5 — actual HabitV5 mechanic: toothpaste -> brush -> face wash -> nails.
...
(() => {
  ...
  function mount(scene){
    ...
    const sync=()=>{
      ...
      pills.forEach((p,i)=>{const active=i===Math.min(st,3),done=i<st||st>=4;p.style.background=active?'#24314a':done?'#b2f7ef':'rgba(255,255,255,.88)';p.style.color=active?'#fff':'#24314a';p.style.border=active?'2px solid #24314a':'2px solid rgba(36,49,74,.11)';p.style.transform=active?'translateY(-2px)':'none';});status.textContent=st>=4?'양치·세수·손톱 정리까지 모두 끝냈어요':STEPS[st][1];
      ...
      progress.style.opacity='1';if(st===0){setFocus(scene.paste?.x||880,scene.paste?.y||570,142,126,true);progress.textContent='치약을 칫솔까지 드래그';scene.hintTarget={x:scene.paste?.x||880,y:scene.paste?.y||570};}else if(st===1){setFocus(790,365,224,154,true);const q=(scene.mouthProgress||[0,0,0,0]).filter(v=>v>=115).length;progress.textContent=`양치 구역 ${q}/4`;scene.hintTarget={x:scene.brush?.x||1040,y:scene.brush?.y||570};}else if(st===2){setFocus(790,330,260,240,true);const n=Math.min(100,Math.round((scene.faceWash||0)/360*100));progress.textContent=`세수 ${n}%`;scene.hintTarget={x:scene.cloth?.x||1110,y:scene.cloth?.y||420};}else if(st===3){setFocus(695,562,170,104,true);progress.textContent=`손톱 ${scene.clipped?.size||0}/5`;scene.hintTarget={x:scene.clipper?.x||1110,y:scene.clipper?.y||270};}else{setFocus(790,365,220,150,false);progress.textContent='모두 완료';}
    };
  ...
  window.__ADUGAME_G1_BENCHMARK_ART_V17__={loaded:true,version:'17.5',generatedVisualAssets:0,directInput:true};
})();
