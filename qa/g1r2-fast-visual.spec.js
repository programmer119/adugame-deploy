...
async function debug(page,name){const d=await page.evaluate(()=>({
  ...
  nailRaiseReady:document.querySelector('#g1r2-v17-overlay')?.dataset.nailRaiseReady,faceWashReady:document.querySelector('#g1r2-v17-overlay')?.dataset.faceWashReady,bathroomBackdropReady:document.querySelector('#g1r2-v17-overlay')?.dataset.bathroomBackdropReady,
...
async function ready(page){
  ...
  await page.waitForFunction(()=>{const r=document.querySelector('#g1r2-v17-overlay');return r?.dataset.ready==='1'&&r?.dataset.nailRaiseReady==='1'&&r?.dataset.faceWashReady==='1'&&r?.dataset.bathroomBackdropReady==='1'&&r?.dataset.version==='17.23'&&window.__ADUGAME_ART_SOURCE__?.G1R2?.version==='v17.23';},null,{timeout:30000});
  await page.waitForTimeout(180);const d=await debug(page,'DEBUG-ready');expect(d.generated).toBe(0);expect(d.version).toBe('v17.23');expect(d.rootVersion).toBe('17.23');expect(d.nailRaiseReady).toBe('1');expect(d.faceWashReady).toBe('1');expect(d.bathroomBackdropReady).toBe('1');expect(d.bathroomBackdrop?.license).toContain('Public Domain');expect(d.state?.benchmarkV5).toBe('brush-face-nails');expect(d.state?.g1r2V17Input).toBeTruthy();
