...
async function washFace(page){
  let before=0;
  for(let i=0;i<6;i++){
    const state=await page.evaluate(()=>window.__ADUGAME_DEBUG__?.());
    if(state?.step===3)return;
    expect(state?.step).toBe(2);
    await drag(page,[[1110,420],[790,330],[735,330],[845,330],[735,345],[845,345],[735,315],[845,315],[790,330]],4);
    await page.waitForTimeout(330);
    const d=await debug(page,`DEBUG-facewash-${i+1}`);
    expect(d.state.faceWash).toBeGreaterThan(before);
    before=d.state.faceWash;
  }
...
test('actual G1R2 v17.3 full brush-face-nails interaction',async({page})=>{
  ...
  await washFace(page);await waitStep(page,3);await snap(page,'STEP-3-nails');
  const nails=[[894,462],[914,452],[936,450],[958,454],[979,464]];
  for(let i=0;i<nails.length;i++){await drag(page,[[1110,270],[1060,330],[1010,390],nails[i]],4);await page.waitForTimeout(250);const state=await page.evaluate(()=>window.__ADUGAME_DEBUG__?.());expect(state.clipped).toBe(i+1);if(i===2)await snap(page,'STEP-3b-nails-3of5');}
