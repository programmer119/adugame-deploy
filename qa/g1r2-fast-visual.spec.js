...
async function dragLogical(page,pts){
  ...
  for(const q of mapped.slice(1))await page.mouse.move(q.x,q.y,{steps:4});
  await page.mouse.up();
}
...
test('actual G1R2 v17.1 visual + full wash interaction',async({page})=>{
  ...
  expect(await page.evaluate(()=>window.__ADUGAME_ART_SOURCE__?.G1R2?.version)).toBe('v17.1');
  await page.screenshot({path:'qa/reports/g1r2-fast/STEP-0-wet.png',fullPage:true});

  await clickLogical(page,475,440);await waitText(page,'비누를 손 위로 가져와요');await page.waitForTimeout(120);
  await page.screenshot({path:'qa/reports/g1r2-fast/STEP-1-soap.png',fullPage:true});

  await dragLogical(page,[[940,486],[860,480],[760,470],[650,455]]);await waitText(page,'손을 좌우로 충분히 문질러요');await page.waitForTimeout(120);
  await page.screenshot({path:'qa/reports/g1r2-fast/STEP-2-scrub.png',fullPage:true});

  const scrub=[[600,455],[700,455],[590,455],[710,455],[590,455],[710,455],[590,455],[710,455]];
  await dragLogical(page,scrub);await waitText(page,'깨끗한 물로 한 번 더 헹궈요');await page.waitForTimeout(120);
  await page.screenshot({path:'qa/reports/g1r2-fast/STEP-3-rinse.png',fullPage:true});

  await clickLogical(page,475,440);await waitText(page,'수건으로 물기를 닦아요');await page.waitForTimeout(120);
  await page.screenshot({path:'qa/reports/g1r2-fast/STEP-4-towel.png',fullPage:true});

  await dragLogical(page,[[265,500],[520,475],[610,455],[700,455],[590,455],[710,455],[590,455],[710,455]]);
  await waitText(page,'깨끗하게 끝!');await page.screenshot({path:'qa/reports/g1r2-fast/STEP-5-done.png',fullPage:true});
});

test('R2 v17.1 initial frame beside benchmark',async({page})=>{
  test.setTimeout(90000);fs.mkdirSync('qa/reports/g1r2-fast',{recursive:true});
  await page.goto('/index.html?game=1&round=2&e2e=1',{waitUntil:'domcontentloaded'});
  await page.waitForFunction(()=>document.querySelector('#g1r2-v17-overlay')?.dataset.ready==='1',null,{timeout:25000});
  await page.waitForTimeout(500);
