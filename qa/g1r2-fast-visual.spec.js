...
const PASTE_HOME=[980,490], BRUSH_PASTE_TARGET=[780,590], BRUSH_HOME=[1000,450], FACE_HOME=[640,260];
...
async function debug(page,name){const d=await page.evaluate(()=>({
  ...
  objects:(()=>{const s=window.__ADUGAME_SCENE__?.();return {paste:s?.paste?{x:s.paste.x,y:s.paste.y,home:s.paste.home}:null,brush:s?.brush?{x:s.brush.x,y:s.brush.y,home:s.brush.home}:null,cloth:s?.cloth?{x:s.cloth.x,y:s.cloth.y,home:s.cloth.home}:null,clipper:s?.clipper?{x:s.clipper.x,y:s.clipper.y}:null,nails:(s?.nails||[]).map(n=>({x:n.x,y:n.y,i:n.nailIndex,active:n.active}))};})()
...
async function ready(page){
  ...
  await page.waitForFunction(()=>{const r=document.querySelector('#g1r2-v17-overlay');return r?.dataset.ready==='1'&&r?.dataset.nailRaiseReady==='1'&&r?.dataset.faceWashReady==='1'&&r?.dataset.version==='17.15'&&window.__ADUGAME_ART_SOURCE__?.G1R2?.version==='v17.15';},null,{timeout:30000});
  await page.waitForTimeout(220);const d=await debug(page,'DEBUG-ready');expect(d.generated).toBe(0);expect(d.version).toBe('v17.15');expect(d.rootVersion).toBe('17.15');expect(d.nailRaiseReady).toBe('1');expect(d.faceWashReady).toBe('1');expect(d.state?.benchmarkV5).toBe('brush-face-nails');expect(d.state?.g1r2V17Input).toBeTruthy();
  expect(d.pasteDisplay).toBe('none');expect(d.brushDisplay).toBe('none');return d;
}
async function brushQuadrant(page,name,pts,index){
  await drag(page,[BRUSH_HOME,...pts]);await page.waitForTimeout(100);const d=await debug(page,`DEBUG-brush-${name}`);expect(d.state.mouthProgress[index]).toBeGreaterThanOrEqual(115);await page.waitForTimeout(240);
}
async function washFaceWithDragEvidence(page){
  ...
  await page.mouse.move(start.x,start.y);await page.mouse.down();await page.mouse.move(first.x,first.y,{steps:2});await page.waitForTimeout(90);
