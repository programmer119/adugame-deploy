...
const NAILS=[[728,276],[741,258],[753,249],[766,254],[779,266]];
...
async function snap(page,name){await page.waitForTimeout(120);await page.screenshot({path:`qa/reports/g1r2-fast/${name}.png`,fullPage:true});}
async function debug(page,name){const d=await page.evaluate(()=>({state:window.__ADUGAME_DEBUG__?.(),version:window.__ADUGAME_ART_SOURCE__?.G1R2?.version,generated:window.__ADUGAME_ART_SOURCE__?.G1R2?.generatedVisualAssets,ready:document.querySelector('#g1r2-v17-overlay')?.dataset.ready,rootVersion:document.querySelector('#g1r2-v17-overlay')?.dataset.version,nailRaiseReady:document.querySelector('#g1r2-v17-overlay')?.dataset.nailRaiseReady,objects:(()=>{const s=window.__ADUGAME_SCENE__?.();return {paste:s?.paste?{x:s.paste.x,y:s.paste.y}:null,brush:s?.brush?{x:s.brush.x,y:s.brush.y}:null,cloth:s?.cloth?{x:s.cloth.x,y:s.cloth.y}:null,clipper:s?.clipper?{x:s.clipper.x,y:s.clipper.y}:null,nails:(s?.nails||[]).map(n=>({x:n.x,y:n.y,i:n.nailIndex,active:n.active}))};})()}));fs.writeFileSync(`qa/reports/g1r2-fast/${name}.json`,JSON.stringify(d,null,2));return d;}
async function ready(page){await page.goto('/index.html?game=1&round=2&e2e=1',{waitUntil:'domcontentloaded'});await page.waitForFunction(()=>{const r=document.querySelector('#g1r2-v17-overlay');return r?.dataset.ready==='1'&&r?.dataset.nailRaiseReady==='1'&&r?.dataset.version==='17.10'&&window.__ADUGAME_ART_SOURCE__?.G1R2?.version==='v17.10';},null,{timeout:30000});await page.waitForTimeout(240);const d=await debug(page,'DEBUG-ready');expect(d.generated).toBe(0);expect(d.version).toBe('v17.10');expect(d.rootVersion).toBe('17.10');expect(d.nailRaiseReady).toBe('1');expect(d.state?.benchmarkV5).toBe('brush-face-nails');expect(d.state?.g1r2V17Input).toBeTruthy();return d;}
...
async function washFaceUntilDone(page){const path=[[1110,420]];for(let i=0;i<4;i++)path.push(...FACE_LOOP);await drag(page,path,1);await waitStep(page,3);const d=await debug(page,'DEBUG-facewash-done');expect(d.state.faceWash).toBeGreaterThanOrEqual(360);return d.state;}

test('R2 v17.10 toothpaste-brush-facewash chain',async({page})=>{
  test.setTimeout(140000);fs.mkdirSync('qa/reports/g1r2-fast',{recursive:true});const start=await ready(page);expect(start.state.step).toBe(0);await snap(page,'STEP-0-toothpaste');
  ...
  const faceDone=await washFaceUntilDone(page);expect(faceDone.step).toBe(3);await snap(page,'STEP-3-nails');const d=await debug(page,'DEBUG-chain-done');expect(d.state.g1r2V17Input.pasteDown).toBe(1);expect(d.state.g1r2V17Input.brushMove).toBeGreaterThan(0);expect(d.state.g1r2V17Input.clothMove).toBeGreaterThan(0);
});

test('R2 v17.10 nail clipper input 5 of 5',async({page})=>{
  test.setTimeout(110000);fs.mkdirSync('qa/reports/g1r2-fast',{recursive:true});await ready(page);
  await page.evaluate(()=>{const s=window.__ADUGAME_SCENE__?.();s.step=3;s.clipped.clear();s.clipper.setPosition(1110,270);s.clipper.home={x:1110,y:270};});await page.waitForTimeout(240);await snap(page,'STEP-3-nails-focused');
  for(let i=0;i<NAILS.length;i++){await drag(page,[[1110,270],[980,270],[880,270],NAILS[i]],1);await page.waitForTimeout(160);const d=await debug(page,`DEBUG-nail-${i+1}`);expect(d.state.clipped).toBe(i+1);if(i===2)await snap(page,'STEP-3b-nails-3of5');}
