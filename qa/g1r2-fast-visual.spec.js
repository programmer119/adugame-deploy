...
test('R2 v17.15 authored-tool interaction + visual evidence',async({page})=>{
  ...
  for(let i=0;i<NAILS.length;i++){await drag(page,[[1110,270],[980,270],[880,270],NAILS[i]]);await page.waitForTimeout(90);const d=await debug(page,`DEBUG-nail-${i+1}`);expect(d.state.clipped).toBe(i+1);if(i===2)await snap(page,'STEP-3b-nails-3of5');}
