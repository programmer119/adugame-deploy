...
async function ready(page){await page.goto('/index.html?game=1&round=2&e2e=1',{waitUntil:'domcontentloaded'});await page.waitForFunction(()=>{const r=document.querySelector('#g1r2-v17-overlay');return r?.dataset.ready==='1'&&r?.dataset.nailRaiseReady==='1'&&r?.dataset.version==='17.9'&&window.__ADUGAME_ART_SOURCE__?.G1R2?.version==='v17.9';},null,{timeout:30000});await page.waitForTimeout(240);const d=await debug(page,'DEBUG-ready');expect(d.generated).toBe(0);expect(d.version).toBe('v17.9');expect(d.rootVersion).toBe('17.9');expect(d.nailRaiseReady).toBe('1');expect(d.state?.benchmarkV5).toBe('brush-face-nails');expect(d.state?.g1r2V17Input).toBeTruthy();return d;}
...
test('R2 v17.9 toothpaste-brush-facewash chain',async({page})=>{
...
test('R2 v17.9 nail clipper input 5 of 5',async({page})=>{
  ...
  for(let i=0;i<NAILS.length;i++){await drag(page,[[1110,270],[1000,340],[880,430],[770,510],NAILS[i]],1);await page.waitForTimeout(160);const d=await debug(page,`DEBUG-nail-${i+1}`);expect(d.state.clipped).toBe(i+1);if(i===2)await snap(page,'STEP-3b-nails-3of5');}
...
test('R2 v17.9 initial frame beside Baby Panda benchmark',async({page})=>{
