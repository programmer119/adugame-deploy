...
async function ready(page){await page.goto('/index.html?game=1&round=2&e2e=1',{waitUntil:'domcontentloaded'});await page.waitForFunction(()=>document.querySelector('#g1r2-v17-overlay')?.dataset.ready==='1',null,{timeout:30000});await page.waitForTimeout(320);const d=await debug(page,'DEBUG-ready');expect(d.generated).toBe(0);expect(d.version).toBe('v17.5');expect(d.state?.benchmarkV5).toBe('brush-face-nails');expect(d.state?.step).toBe(0);expect(d.state?.g1r2V17Input).toBeTruthy();}
async function brushQuadrant(page,name,pts,index){await drag(page,[[1040,570],...pts],2);await page.waitForTimeout(280);const d=await debug(page,`DEBUG-brush-${name}`);expect(d.state.mouthProgress[index]).toBeGreaterThanOrEqual(115);}
async function washFace(page){let before=0;for(let i=0;i<4;i++){const state=await page.evaluate(()=>window.__ADUGAME_DEBUG__?.());if(state?.step===3)return;expect(state?.step).toBe(2);await drag(page,[[1110,420],[790,330],[735,330],[845,330],[735,345],[845,345],[735,315],[845,315],[790,330]],3);await page.waitForTimeout(260);const d=await debug(page,`DEBUG-facewash-${i+1}`);expect(d.state.faceWash).toBeGreaterThan(before);before=d.state.faceWash;}await waitStep(page,3);}

test('actual G1R2 v17.5 full brush-face-nails interaction',async({page})=>{
  ...
  await drag(page,[[880,570],[930,570],[985,570],[1040,570]],3);await waitStep(page,1);await snap(page,'STEP-1-brush');
  ...
  for(let i=0;i<nails.length;i++){
    await drag(page,[[1110,270],[1030,330],[920,410],[810,500],nails[i]],3);await page.waitForTimeout(240);
    const d=await debug(page,`DEBUG-nail-${i+1}`);expect(d.state.clipped).toBe(i+1);if(i===2)await snap(page,'STEP-3b-nails-3of5');
  }
  await waitStep(page,4);await snap(page,'STEP-4-done');const done=await debug(page,'DEBUG-done');
  expect(done.state.g1r2V17Input.pasteDown).toBeGreaterThan(0);expect(done.state.g1r2V17Input.brushMove).toBeGreaterThan(0);expect(done.state.g1r2V17Input.clothMove).toBeGreaterThan(0);expect(done.state.g1r2V17Input.clipperUp).toBeGreaterThanOrEqual(5);
});

test('R2 v17.5 initial frame beside Baby Panda benchmark',async({page})=>{
