...
test('R2 v17.8 toothpaste-brush-facewash chain',async({page})=>{
...
test('R2 v17.8 nail clipper input 5 of 5',async({page})=>{
  test.setTimeout(95000);fs.mkdirSync('qa/reports/g1r2-fast',{recursive:true});await ready(page);
  await page.evaluate(()=>{const s=window.__ADUGAME_SCENE__?.();s.step=3;s.clipped.clear();s.clipper.setPosition(1110,270);s.clipper.home={x:1110,y:270};});await page.waitForTimeout(220);await snap(page,'STEP-3-nails-focused');
...
test('R2 v17.8 initial frame beside Baby Panda benchmark',async({page})=>{
