...
const NAILS=[[776,226],[791,204],[805,194],[821,200],[836,214]];
const PASTE_HOME=[980,490], BRUSH_PASTE_TARGET=[780,590], BRUSH_HOME=[1000,450], FACE_HOME=[640,260], CLIPPER_HOME=[900,260];
...
async function ready(page){
  ...
  await page.waitForFunction(()=>{const r=document.querySelector('#g1r2-v17-overlay');return r?.dataset.ready==='1'&&r?.dataset.nailRaiseReady==='1'&&r?.dataset.faceWashReady==='1'&&r?.dataset.version==='17.21'&&window.__ADUGAME_ART_SOURCE__?.G1R2?.version==='v17.21';},null,{timeout:30000});
  await page.waitForTimeout(180);const d=await debug(page,'DEBUG-ready');expect(d.generated).toBe(0);expect(d.version).toBe('v17.21');expect(d.rootVersion).toBe('17.21');expect(d.nailRaiseReady).toBe('1');expect(d.faceWashReady).toBe('1');expect(d.state?.benchmarkV5).toBe('brush-face-nails');expect(d.state?.g1r2V17Input).toBeTruthy();
...
test('R2 v17.21 immersive authored-tool interaction + visual evidence',async({page})=>{
  ...
  for(let i=0;i<NAILS.length;i++){await drag(page,[CLIPPER_HOME,[860,245],[840,225],NAILS[i]]);await page.waitForTimeout(90);const d=await debug(page,`DEBUG-nail-${i+1}`);expect(d.state.clipped).toBe(i+1);if(i===2)await snap(page,'STEP-3b-nails-3of5');}
