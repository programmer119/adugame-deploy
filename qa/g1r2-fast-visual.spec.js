...
async function ready(page){
  ...
  await page.waitForFunction(()=>document.querySelector('#g1r2-v17-overlay')?.dataset.ready==='1',null,{timeout:30000});await page.waitForTimeout(350);
...
async function brushQuadrant(page,name,pts,index){
  await drag(page,[[1070,555],...pts],3);await page.waitForTimeout(320);
  const d=await debug(page,`DEBUG-brush-${name}`);expect(d.state.mouthProgress[index]).toBeGreaterThanOrEqual(115);
}
...
test('actual G1R2 v17.3 full brush-face-nails interaction',async({page})=>{
  ...
  const washing=[[1060,555],[750,330],[840,330],[750,345],[840,345],[750,315],[840,315],[750,345],[840,345],[750,330]];
  ...
  for(let i=0;i<nails.length;i++){
    await drag(page,[[1070,555],[1020,520],nails[i]],4);await page.waitForTimeout(240);
