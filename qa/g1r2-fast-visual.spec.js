...
async function debug(page,name){const d=await page.evaluate(()=>({state:window.__ADUGAME_DEBUG__?.(),version:window.__ADUGAME_ART_SOURCE__?.G1R2?.version,generated:window.__ADUGAME_ART_SOURCE__?.G1R2?.generatedVisualAssets,ready:document.querySelector('#g1r2-v17-overlay')?.dataset.ready,rootVersion:document.querySelector('#g1r2-v17-overlay')?.dataset.version,nailRaiseReady:document.querySelector('#g1r2-v17-overlay')?.dataset.nailRaiseReady,faceWashReady:document.querySelector('#g1r2-v17-overlay')?.dataset.faceWashReady,clothOpacity:document.querySelector('.g1v17-cloth')?.style.opacity,objects:(()=>{const s=window.__ADUGAME_SCENE__?.();return {paste:s?.paste?{x:s.paste.x,y:s.paste.y}:null,brush:s?.brush?{x:s.brush.x,y:s.brush.y}:null,cloth:s?.cloth?{x:s.cloth.x,y:s.cloth.y}:null,clipper:s?.clipper?{x:s.clipper.x,y:s.clipper.y}:null,nails:(s?.nails||[]).map(n=>({x:n.x,y:n.y,i:n.nailIndex,active:n.active}))};})()}));fs.writeFileSync(`qa/reports/g1r2-fast/${name}.json`,JSON.stringify(d,null,2));return d;}
async function ready(page){await page.goto('/index.html?game=1&round=2&e2e=1',{waitUntil:'domcontentloaded'});await page.waitForFunction(()=>{const r=document.querySelector('#g1r2-v17-overlay');return r?.dataset.ready==='1'&&r?.dataset.nailRaiseReady==='1'&&r?.dataset.faceWashReady==='1'&&r?.dataset.version==='17.14'&&window.__ADUGAME_ART_SOURCE__?.G1R2?.version==='v17.14';},null,{timeout:30000});await page.waitForTimeout(180);const d=await debug(page,'DEBUG-ready');expect(d.generated).toBe(0);expect(d.version).toBe('v17.14');expect(d.rootVersion).toBe('17.14');expect(d.nailRaiseReady).toBe('1');expect(d.faceWashReady).toBe('1');expect(d.state?.benchmarkV5).toBe('brush-face-nails');expect(d.state?.g1r2V17Input).toBeTruthy();return d;}
async function brushQuadrant(page,name,pts,index){await drag(page,[[1040,570],...pts]);await page.waitForTimeout(100);const d=await debug(page,`DEBUG-brush-${name}`);expect(d.state.mouthProgress[index]).toBeGreaterThanOrEqual(115);}
async function washFaceWithDragEvidence(page){
  ...
  await page.mouse.move(start.x,start.y);await page.mouse.down();await page.mouse.move(first.x,first.y,{steps:2});await page.waitForTimeout(80);
  const moving=await debug(page,'DEBUG-facewash-dragging');expect(moving.state.g1r2V17Input.active).toBe('cloth');expect(Number(moving.clothOpacity)).toBeGreaterThan(.5);await snap(page,'STEP-2b-facewash-dragging');
...
test('R2 v17.14 full interaction + visual evidence',async({page})=>{
  ...
  await waitStep(page,2);const atFace=await debug(page,'DEBUG-facewash-ready');expect(Math.abs(atFace.objects.cloth.x-FACE_HOME[0])).toBeLessThan(4);expect(Math.abs(atFace.objects.cloth.y-FACE_HOME[1])).toBeLessThan(4);expect(Number(atFace.clothOpacity||0)).toBeLessThan(.1);await snap(page,'STEP-2-facewash');
  await washFaceWithDragEvidence(page);await snap(page,'STEP-3-nails-focused');
  for(let i=0;i<NAILS.length;i++){await drag(page,[[1110,270],[980,270],[880,270],NAILS[i]]);await page.waitForTimeout(120);const d=await debug(page,`DEBUG-nail-${i+1}`);expect(d.state.clipped).toBe(i+1);if(i===2)await snap(page,'STEP-3b-nails-3of5');}
