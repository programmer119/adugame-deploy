...
test('R2 v17.10 full interaction + visual evidence',async({page})=>{
  ...
  const facePath=[[1110,420]];for(let i=0;i<4;i++)facePath.push(...FACE_LOOP);await drag(page,facePath);await waitStep(page,3);const face=await debug(page,'DEBUG-facewash-done');expect(face.state.faceWash).toBeGreaterThanOrEqual(360);await snap(page,'STEP-3-nails-focused');
