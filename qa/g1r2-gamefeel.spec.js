...
async function gf(page){return page.evaluate(()=>{
  const root=document.querySelector('#g1r2-v17-overlay'),rail=document.querySelector('.g1v17-gamefeel-rail'),fill=document.querySelector('.g1v17-gamefeel-fill');
  ...
  return{
    ...
    rail:{display:rail?getComputedStyle(rail).display:null,opacity:rail?parseFloat(getComputedStyle(rail).opacity):0,mode:rail?.dataset.mode||'',pulse:rail?.dataset.pulse||'',text:rail?.firstElementChild?.textContent||'',box:rr?{left:rr.left,top:rr.top,right:rr.right,bottom:rr.bottom}:null},
...
async function ready(page){
  ...
  await page.waitForFunction(()=>{const r=document.querySelector('#g1r2-v17-overlay');return r?.dataset.gameFeelReady==='1'&&r?.dataset.motionReady==='1'&&r?.dataset.toolHitAlignmentReady==='1'&&r?.dataset.version==='17.32'&&window.__ADUGAME_ART_SOURCE__?.G1R2?.version==='v17.32';},null,{timeout:60000});
  const d=await gf(page);expect(d.generated).toBe(0);expect(d.meta?.stableVisualProgress).toBe(true);expect(d.meta?.milestoneRailPulse).toBe(true);expect(d.meta?.noFloatingBadge).toBe(true);expect(d.meta?.brushFeedbackOnRelease).toBe(true);expect(d.meta?.nailFeedbackOnRelease).toBe(true);expect(d.meta?.donePersistent).toBe(true);expect(d.meta?.mobileCompact).toBe(true);expect(d.meta?.progressDomUpdatesOnChangeOnly).toBe(true);return d;
}
...
test('R2 v17.32 desktop stable progress + rail success feedback',async({page})=>{
  ...
  await brushQuadrant(page,0);await page.waitForTimeout(130);d=await gf(page);expect(d.railState).toBe('brush:1/4');expect(d.segments).toEqual(['1','0','0','0']);expect(d.lastPulse).toBe('brush:1');expect(d.rail.text).toContain('양치 1/4');expect(d.rail.opacity).toBeGreaterThan(.8);await snap(page,'GAMEFEEL-1b-brush-success.png');
  ...
  await drag(page,[CLIPPER_HOME,[950,235],NAILS[0]],2);await page.waitForFunction(()=>window.__ADUGAME_DEBUG__?.()?.clipped===1,null,{timeout:3000});await page.waitForTimeout(130);d=await gf(page);expect(d.railState).toBe('nails:1/5');expect(d.segments).toEqual(['1','0','0','0','0']);expect(d.lastPulse).toBe('nail:1');expect(d.rail.text).toContain('손톱 1/5');expect(d.rail.opacity).toBeGreaterThan(.8);await snap(page,'GAMEFEEL-3-nail-success.png');
  for(let i=1;i<NAILS.length;i++){await drag(page,[CLIPPER_HOME,[950,235],NAILS[i]],2);await page.waitForTimeout(65);}await waitStep(page,4);await page.waitForTimeout(130);d=await gf(page);expect(d.state.roundComplete).toBe(true);expect(d.lastPulse).toBe('done');expect(d.lastText).toBe('모두 완료 ✓');expect(d.railState).toBe('done');expect(d.rail.mode).toBe('done');expect(d.rail.text).toBe('모두 완료 ✓');expect(d.rail.opacity).toBeGreaterThan(.8);expect(d.segments).toEqual(['1','1','1','1']);expect(d.pulseCount).toBeGreaterThanOrEqual(6);expect(d.generated).toBe(0);await snap(page,'GAMEFEEL-4-complete.png');
});

test('R2 v17.32 compact mobile keeps added HUD out of action area',async({page})=>{
  test.setTimeout(120000);fs.mkdirSync('qa/reports/g1r2-fast',{recursive:true});await page.setViewportSize({width:844,height:390});let d=await ready(page);expect(d.layout.scrollWidth).toBeLessThanOrEqual(d.layout.innerWidth+2);expect(d.rail.display).toBe('none');await snap(page,'GAMEFEEL-MOBILE-0-clean.png');
  await drag(page,[PASTE_HOME,[1110,560],PASTE_TARGET],2);await waitStep(page,1);d=await gf(page);expect(d.railState).toBe('brush:0/4');expect(d.rail.display).toBe('none');await page.emulateMedia({reducedMotion:'reduce'});await page.waitForFunction(()=>document.querySelector('#g1r2-v17-overlay')?.dataset.motionReduced==='1',null,{timeout:2500});await brushQuadrant(page,0);d=await gf(page);expect(d.state.mouthProgress[0]).toBeGreaterThanOrEqual(115);expect(d.lastPulse).toBe('brush:1');expect(d.rail.display).toBe('none');expect(d.generated).toBe(0);await snap(page,'GAMEFEEL-MOBILE-1-reduced-clean.png');
});
