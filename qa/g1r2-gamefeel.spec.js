...
async function brushQuadrant(page,index){
  const pts=BRUSH_PATHS[index];await beginDrag(page,[BRUSH_HOME,pts[0]],2);for(const a of pts.slice(1)){const q=await p(page,a[0],a[1]);await page.mouse.move(q.x,q.y,{steps:1});}await page.mouse.up();await page.waitForTimeout(75);await page.waitForFunction(i=>window.__ADUGAME_DEBUG__?.()?.mouthProgress?.[i]>=115,index,{timeout:3000});
}
...
test('R2 v17.31 desktop visible progress + success milestones',async({page})=>{
  ...
  await drag(page,[PASTE_HOME,[1110,560],PASTE_TARGET],2);await waitStep(page,1);d=await gf(page);expect(d.railState).toBe('brush:0/4');expect(d.rail.opacity).toBeGreaterThan(.8);expect(d.rail.text).toContain('양치 0/4');expect(d.segments).toEqual(['0','0','0','0']);await snap(page,'GAMEFEEL-1-brush-rail.png');
  await brushQuadrant(page,0);d=await gf(page);expect(d.railState).toBe('brush:1/4');expect(d.segments).toEqual(['1','0','0','0']);expect(d.lastPulse).toBe('brush:1');expect(d.lastText).toContain('양치 1/4');expect(d.badge.opacity).toBeGreaterThan(.5);await snap(page,'GAMEFEEL-1b-brush-success.png');
  await brushQuadrant(page,1);await brushQuadrant(page,2);await brushQuadrant(page,3);await waitStep(page,2);d=await gf(page);expect(d.rail.mode).toBe('wash');expect(d.rail.text).toContain('세수');await snap(page,'GAMEFEEL-2-facewash-rail.png');
  const start=await p(page,FACE_HOME[0],FACE_HOME[1]);await page.mouse.move(start.x,start.y);await page.mouse.down();for(let i=0;i<4;i++){for(const [x,y] of FACE_LOOP){const q=await p(page,x,y);await page.mouse.move(q.x,q.y,{steps:1});if(i===0&&x===790&&y===330){const mid=await gf(page);expect(mid.fill).not.toBeNull();}}}d=await gf(page);expect(d.fill).toBeGreaterThan(0);await snap(page,'GAMEFEEL-2b-facewash-progress.png');await page.mouse.up();await waitStep(page,3);
  d=await gf(page);expect(d.railState).toBe('nails:0/5');expect(d.segments).toEqual(['0','0','0','0','0']);
  await drag(page,[CLIPPER_HOME,[950,235],NAILS[0]],2);await page.waitForFunction(()=>window.__ADUGAME_DEBUG__?.()?.clipped===1,null,{timeout:3000});d=await gf(page);expect(d.railState).toBe('nails:1/5');expect(d.segments).toEqual(['1','0','0','0','0']);expect(d.lastPulse).toBe('nail:1');expect(d.badge.opacity).toBeGreaterThan(.5);await snap(page,'GAMEFEEL-3-nail-success.png');
  for(let i=1;i<NAILS.length;i++){await drag(page,[CLIPPER_HOME,[950,235],NAILS[i]],2);await page.waitForTimeout(65);}await waitStep(page,4);d=await gf(page);expect(d.state.roundComplete).toBe(true);expect(d.lastPulse).toBe('done');expect(d.lastText).toBe('모두 완료 ✓');expect(d.badge.text).toBe('모두 완료 ✓');expect(d.badge.opacity).toBeGreaterThan(.5);expect(d.pulseCount).toBeGreaterThanOrEqual(6);expect(d.generated).toBe(0);await snap(page,'GAMEFEEL-4-complete.png');
});
