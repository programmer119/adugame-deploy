...
async function waitTransition(page,kind='active'){await page.waitForFunction(k=>document.querySelector('#g1r2-v17-overlay')?.dataset.motionTransition===k,kind,{timeout:1200});}
...
async function debug(page,name){
  const d=await page.evaluate(()=>{
    ...
    return{
      ...
      motionReady:root?.dataset.motionReady,motionTransition:root?.dataset.motionTransition,motionTransitionCount:Number(root?.dataset.motionTransitionCount||0),motionNearTarget:root?.dataset.motionNearTarget||'',motionReduced:root?.dataset.motionReduced,
      ...
      layout:{innerWidth:innerWidth,innerHeight:innerHeight,scrollWidth:document.documentElement.scrollWidth,scrollHeight:document.documentElement.scrollHeight,canvas:rect(document.querySelector('canvas')),root:rect(root)},
...
async function ready(page,label='ready'){
  ...
  expect(d.generated).toBe(0);expect(d.motionMeta?.stepTransition).toBe(true);expect(d.motionMeta?.targetProximityFeedback).toBe(true);expect(d.motionMeta?.reducedMotionAware).toBe(true);
...
test('R2 v17.30 desktop full dynamic UX + target feedback',async({page})=>{
  ...
  const tgt=await p(page,BRUSH_PASTE_TARGET[0],BRUSH_PASTE_TARGET[1]);await page.mouse.move(tgt.x,tgt.y,{steps:2});await page.mouse.up();await waitStep(page,1);await waitTransition(page,'active');await snap(page,'STEP-0d-transition-to-brush');await page.waitForTimeout(260);let b=await debug(page,'DEBUG-brush-ready');expect(Math.abs(b.objects.brush.x-BRUSH_HOME[0])).toBeLessThan(3);expect(b.motionTransitionCount).toBeGreaterThanOrEqual(1);await snap(page,'STEP-1-brush');
  await brushQuadrant(page,0,true);await brushQuadrant(page,1);await brushQuadrant(page,2);await brushQuadrant(page,3);await waitStep(page,2);await waitTransition(page,'active');await page.waitForTimeout(150);const f=await debug(page,'DEBUG-facewash-ready');expect(Math.abs(f.objects.cloth.x-FACE_HOME[0])).toBeLessThan(3);expect(Math.abs(f.objects.cloth.y-FACE_HOME[1])).toBeLessThan(3);expect(f.faceVisual.left).toBe('54.6%');expect(f.faceVisual.top).toBe('33.6%');expect(f.faceVisual.width).toBe('78%');expect(f.faceVisual.height).toBe('90%');await snap(page,'STEP-2-facewash');
  await washFace(page);await waitTransition(page,'active');await page.waitForTimeout(100);const n=await debug(page,'DEBUG-nails-ready');expect(Math.abs(n.objects.clipper.x-CLIPPER_HOME[0])).toBeLessThan(4);expect(n.nailGuides.map(x=>x.done)).toEqual(['0','0','0','0','0']);await snap(page,'STEP-3-nails-focused');
  ...
  await waitStep(page,4);await waitTransition(page,'active');await page.waitForTimeout(110);await snap(page,'STEP-4-done');const done=await debug(page,'DEBUG-done');expect(done.state.mouthProgress.every(v=>v>=115)).toBe(true);expect(done.state.faceWash).toBeGreaterThanOrEqual(360);expect(done.state.clipped).toBe(5);expect(done.state.roundComplete).toBe(true);expect(done.motionTransitionCount).toBeGreaterThanOrEqual(4);expect(done.state.g1r2V17Input.pasteDown).toBeGreaterThanOrEqual(2);expect(done.state.g1r2V17Input.brushMove).toBeGreaterThan(0);expect(done.state.g1r2V17Input.clothMove).toBeGreaterThan(0);expect(done.state.g1r2V17Input.clipperUp).toBeGreaterThanOrEqual(5);expect(done.generated).toBe(0);
});

test('R2 v17.30 mobile landscape scaled input + reduced motion',async({page})=>{
  ...
  expect(start.layout.scrollWidth).toBeLessThanOrEqual(start.layout.innerWidth+2);expect(start.layout.canvas.width).toBeGreaterThan(300);expect(start.layout.canvas.height).toBeGreaterThan(160);expect(start.readability.progress.inside).toBe(true);expect(start.readability.status.inside).toBe(true);await snap(page,'MOBILE-0-toothpaste');
  await beginDrag(page,[PASTE_HOME,[1045,620]],2);await page.waitForTimeout(50);let near=await debug(page,'DEBUG-mobile-paste-near');expect(near.motionNearTarget).toBe('paste');const tgt=await p(page,BRUSH_PASTE_TARGET[0],BRUSH_PASTE_TARGET[1]);await page.mouse.move(tgt.x,tgt.y,{steps:2});await page.mouse.up();await waitStep(page,1);await waitTransition(page,'active');await page.waitForTimeout(220);let step1=await debug(page,'DEBUG-mobile-brush-ready');expect(step1.state.step).toBe(1);expect(Math.abs(step1.objects.brush.x-BRUSH_HOME[0])).toBeLessThan(3);await snap(page,'MOBILE-1-brush');
  ...
  await brushQuadrant(page,0,true,'MOBILE-');await brushQuadrant(page,1,false,'MOBILE-');await brushQuadrant(page,2,false,'MOBILE-');await brushQuadrant(page,3,false,'MOBILE-');await waitStep(page,2);await waitTransition(page,'reduced');const reducedState=await debug(page,'DEBUG-mobile-reduced-transition');expect(reducedState.motionReduced).toBe('1');expect(reducedState.state.step).toBe(2);expect(reducedState.generated).toBe(0);await snap(page,'MOBILE-2-reduced-motion-facewash');
});
