...
async function waitScene(page){await page.waitForFunction(()=>window.__ADUGAME_SCENE__?.()?.v6Visual&&window.__ADUGAME_VISUAL_V6_POLISH__?.loaded,{timeout:10000});}
...
for(let g=1;g<=3;g++)for(let r=1;r<=3;r++){
  test(`v6 visual quality start G${g}R${r}`,async({page})=>{
    ...
    if(g===2){const st=await page.evaluate(()=>{const s=window.__ADUGAME_SCENE__(),fixtures=[s.car,s.toolbox,s.yardBox,s.stove,s.sink,s.fridge,s.sofa,s.washer,s.rack,s.bath,s.toyBox,s.bed,s.patio].filter(Boolean);return{fixtures:fixtures.every(o=>o.visualIdentity==='illustrated'),items:(s.items||[]).every(o=>o.visualIdentity==='scene-shelf-item'),chars:(s.characters||[]).every(o=>o.visualIdentity==='illustrated-character'),elevator:s.elevator?.visualIdentity,polish:s.v6Polish};});expect(st.fixtures).toBe(true);expect(st.items).toBe(true);expect(st.chars).toBe(true);expect(st.elevator).toBe('illustrated-elevator');expect(st.polish).toContain('house-floor-');}
