...
async function guide(p){return p.evaluate(()=>{const s=window.__ADUGAME_SCENE__();return{status:String(s.status?.text||''),hint:s.hintTarget?{x:s.hintTarget.x,y:s.hintTarget.y}:null,guard:window.__ADUGAME_CLARITY_G3_STABLE_V5__||null};});}

test('strict command chain: G3 wrong container never advertises serve as the next valid action',async({page})=>{
