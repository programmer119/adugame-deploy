...
test('strict command chain: G3 invalid choices recover to the next valid action',async({page})=>{
  ...
  await clickL(page,r,870,630);await page.waitForTimeout(120);let g=await guide(page);expect(g.guard?.failedServeRecovery).toBe(true);expect(g.status).toContain('베이스');expect(g.status).toContain('활성액');expect(Math.abs(g.hint.x-230)).toBeLessThanOrEqual(3);expect(Math.abs(g.hint.y-230)).toBeLessThanOrEqual(3);
  await clickL(page,r,380,585);await page.waitForTimeout(120);g=await guide(page);expect(g.guard?.wrongContainerGuard).toBe(true);expect(g.status).toContain('베이스');expect(g.status).toContain('활성액');expect(g.status).not.toContain('모두 맞췄');expect(Math.abs(g.hint.x-230)).toBeLessThanOrEqual(3);expect(Math.abs(g.hint.y-230)).toBeLessThanOrEqual(3);
