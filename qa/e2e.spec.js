...
async function clickL(page,r,x,y){ const p=map(r,x,y); await page.mouse.click(p.x,p.y,{delay:80}); }
async function dragPath(page,r,points,pauseMs=0){
  const ps=points.map(([x,y])=>map(r,x,y));
  await page.mouse.move(ps[0].x,ps[0].y);
  await page.mouse.down();
  // Let the 80ms pickup/lift animation finish so drag coordinates are not racing that tween.
...
async function expectComplete(page,errors,label){
  try{ await page.waitForFunction(()=>window.__ADUGAME_DEBUG__()?.roundComplete===true,null,{timeout:6000}); }
...
const cases=[
  ...
  [1,2,async(p,r)=>{
    ...
    await dragPath(p,r,[[315,350],[650,455]]); await settle(p,260);
    await dragPath(p,r,[[590,455],[710,455],[590,455],[710,455],[590,455],[710,455]],40); await settle(p,100);
    await clickL(p,r,650,270); await settle(p,560);
    await dragPath(p,r,[[315,500],[570,455],[730,455],[570,455],[730,455]],40);
  }],
  [1,3,async(p,r)=>{
    await clickL(p,r,180,250); await settle(p,120);
    await dragPath(p,r,[[250,255],[350,255],[250,255],[350,255],[250,255]],40); await settle(p,120);
    await dragPath(p,r,[[210,405],[500,390],[900,390],[500,500],[900,500]],40); await settle(p,260);
    await dragPath(p,r,[[650,630],[730,470]]); await settle(p,260);
    for(const q of [[820,625],[930,625],[1040,625]]){ await dragPath(p,r,[q,[730,450]]); await settle(p,260); }
    await dragPath(p,r,[[1120,500],[1120,390]]);
  }],
  [2,1,async(p,r)=>{
    for(const q of [[500,610],[620,620],[740,620]]){ await dragPath(p,r,[q,[710,500]]); await settle(p,260); }
  }],
  [2,2,async(p,r)=>{
    await clickL(p,r,650,355); await settle(p,120); // open
    for(const q of [[305,220],[305,330],[305,440],[1000,390]]){ await dragPath(p,r,[q,[650,355]]); await settle(p,260); }
    // Loaded objects visually overlap the door center in v2; close on the exposed lower edge of the door.
    await clickL(p,r,650,425); await settle(p,120);
    await clickL(p,r,718,235); // start
    await p.waitForTimeout(1750);
    for(const q of [[760,585],[838,585],[916,585]]){ await dragPath(p,r,[q,[925,500]]); await settle(p,260); }
  }],
  [2,3,async(p,r)=>{
    await dragPath(p,r,[[300,210],[575,465]]); await settle(p,260);
    await dragPath(p,r,[[300,330],[575,465]]); await settle(p,260);
    await dragPath(p,r,[[300,455],[575,465]]); await settle(p,260);
    await dragPath(p,r,quarterCircle(575,465,60,2),80);
  }],
  [3,1,async(p,r)=>{
    await dragPath(p,r,[[230,230],[650,420]]); await settle(p,300);
    await dragPath(p,r,[[360,230],[650,420]]); await settle(p,360);
    await clickL(p,r,210,355); await settle(p,140);
    await dragPath(p,r,mixPath(650,420,82,2.75),150); await settle(p,300);
    await dragPath(p,r,[[210,485],[650,420]]); await settle(p,260);
  ...
  [3,2,async(p,r)=>{
    ...
    await dragPath(p,r,mixPath(650,420,82,2.75),150); await settle(p,300);
    await dragPath(p,r,[[305,485],[650,420]]); await settle(p,260);
  ...
  [3,3,async(p,r)=>{
    ...
    await clickL(p,r,210,355); await settle(p,140);
    await dragPath(p,r,mixPath(650,420,82,2.75),150); await settle(p,300);
    await dragPath(p,r,[[210,485],[650,420]]); await settle(p,240);
    await dragPath(p,r,[[495,485],[690,440]]); await settle(p,240);
    await clickL(p,r,870,630);
  }]
];
