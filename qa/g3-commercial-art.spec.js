...
test('G3 authored commercial art stays aligned through first customer and shop reveal',async({page})=>{
  ...
  await waitDebug(page,()=>{const r=document.getElementById('g3-commercial-art-v1');return r&&r.dataset.ready==='1'&&r.dataset.hitAlignment==='1'&&r.dataset.v2HeroReady==='1'&&r.dataset.version==='2.0';},null,30000);
  const r=await page.locator('canvas').boundingBox();expect(r).toBeTruthy();
  const meta=await page.evaluate(()=>({root:{...document.getElementById('g3-commercial-art-v1').dataset},art:window.__ADUGAME_ART_SOURCE__?.G3,imgs:[...document.querySelectorAll('#g3-commercial-art-v1 img')].map(x=>({cls:x.className,naturalWidth:x.naturalWidth,naturalHeight:x.naturalHeight,src:x.src}))}));
  expect(meta.root.generatedVisualAssets).toBe('0');expect(meta.root.artQuality).toBe('authored-scene-v2');expect(meta.art.generatedVisualAssets).toBe(0);expect(meta.art.version).toBe('commercial-v2');expect(meta.imgs.filter(x=>x.naturalWidth>0).length).toBeGreaterThanOrEqual(6);expect(meta.imgs.find(x=>x.cls==='g3-clerk')?.src).toContain('/300873');
  ...
  await page.waitForTimeout(220);expect(await page.locator('.g3-slime').evaluate(e=>getComputedStyle(e).opacity)).toBe('1');
  ...
  await page.waitForTimeout(420);await shot(page,'G3-4-next-customer.png');
  ...
  expect(end.debug.coinBalance).toBe(1);expect(end.debug.supplyStock.soccer).toBe(2);expect(end.root.generatedVisualAssets).toBe('0');expect(end.art.generatedVisualAssets).toBe(0);
