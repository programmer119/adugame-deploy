// ADUGAME G3 commercial-art v2.31 — ready-order input invariant.
// A valid, unlocked order must always have an actionable live serve button.
(() => {
  ...
  function attach(scene){
    ...
    if(priorDebug&&!scene.__g3ServeDebugWrapped){
      ...
      scene.debugState=function(){return {...priorDebug(),serveReadyInvariant:ready(scene),serveInputEnabled:!!scene.serveButton?.input?.enabled,serveReadyRepairCount:scene.g3ServeReadyRepairCount||0};};
    }
    const cleanup=()=>{scene.__g3ServeReadyGuard=false;};scene.events.once('shutdown',cleanup);scene.events.once('destroy',cleanup);
  }
  ...
  window.__ADUGAME_G3_COMMERCIAL_ART_V2_SERVE_GUARD__={loaded:true,version:'2.31',readyOrderInputInvariant:true,generatedVisualAssets:0};
})();