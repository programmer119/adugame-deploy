// ADUGAME G3 commercial-art v2.33 — ready-order input invariant.
...
(() => {
  ...
  function attach(scene){
    ...
    if(b&&!b.__g3DisableGuarded&&typeof b.disableInteractive==='function'){
      ...
      b.disableInteractive=function(...args){
        if(ready(scene)&&!scene.__g3ServeDisableAllowed){
          ...
          if(!this.input?.enabled)this.setInteractive({useHandCursor:true});
          return this;
        }
    ...
    const sync=()=>{
      ...
      const live=scene.serveButton;
      if(ready(scene)&&live&&live.visible!==false&&!live.input?.enabled){
    ...
    if(priorDebug&&!scene.__g3ServeDebugWrapped){
      ...
      scene.debugState=function(){return {...priorDebug(),serveReadyInvariant:ready(scene),serveInputEnabled:!!scene.serveButton?.input?.enabled,serveReadyRepairCount:scene.g3ServeReadyRepairCount||0,serveReadyBlockedDisableCount:scene.g3ServeReadyBlockedDisableCount||0,serveReadyLastRepairAt:scene.g3ServeReadyLastRepairAt||0,serveReadyLastBlockedAt:scene.g3ServeReadyLastBlockedAt||0};};
    }
    const cleanup=()=>{
  ...
  window.__ADUGAME_G3_COMMERCIAL_ART_V2_SERVE_GUARD__={loaded:true,version:'2.33',readyOrderInputInvariant:true,blockedForeignDisable:true,realServeDisableAllowed:true,postStepInvariant:true,generatedVisualAssets:0};
})();