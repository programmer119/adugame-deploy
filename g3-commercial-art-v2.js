// ADUGAME G3 commercial-art v2.3 — cohesive authored scene pass + resolved completion state.
...
(() => {
  ...
  function attach(scene){
    ...
    scene.__g3CommercialV2=true;root.dataset.version='2.3';root.dataset.qualityTarget='g1r2-commercial';root.dataset.visualPolish='v2.3';

    const maker=root.querySelector('.g3-clerk'),customer=root.querySelector('.g3-shoppers'),bowl=root.querySelector('.g3-bowl'),slime=root.querySelector('.g3-slime'),shelf=root.querySelector('.g3-shelf'),counter=root.querySelector('.g3-workbench'),awning=root.querySelector('.g3-shop-awning'),title=root.querySelector('.g3-title'),order=root.querySelector('.g3-order-card'),status=root.querySelector('.g3-status'),round=root.querySelector('.g3-round'),focus=root.querySelector('.g3-focus'),coins=root.querySelector('.g3-coins'),serveDom=root.querySelector('.g3-serve');
    ...
    if(order)Object.assign(order.style,{right:'2.2%',top:'9.4%',width:'15.5%',minHeight:'84px',padding:'11px 10px',borderRadius:'21px',background:'rgba(255,255,255,.93)',border:'3px solid rgba(226,142,170,.38)',boxShadow:'0 8px 18px rgba(92,56,78,.09)',transition:'opacity .18s ease,filter .18s ease'});
    ...
    if(serveDom)Object.assign(serveDom.style,{transition:'opacity .16s ease,transform .16s ease,filter .14s ease,box-shadow .14s ease'});
    ...
    // Interaction remains owned by Phaser while the round is live. Once complete, remove the stale submit affordance entirely.
    ...
    const sync=()=>{
      ...
      const mixed=!!scene.mixed,ready=root.dataset.orderReady==='true',ingredients=scene.ingredients instanceof Set?[...scene.ingredients]:(scene.ingredients||[]),chosen=scene.chosen||{},activeOrder=scene.order||{},orders=Number(scene.ordersServed||0),total=Number(scene.orders?.length||2),coin=Number(scene.coinCount||0),complete=!!scene.roundComplete||orders>=total;
      if(maker)maker.style.filter=mixed?'saturate(1.07) contrast(1.02)':'saturate(1.04) contrast(1.015)';
      if(customer)customer.style.transform=`translate(-50%,-50%) scale(${ready&&!complete?1.018:1})`;
      ...
      if(complete){
        if(scene.serveButton){scene.serveButton.setVisible(false);scene.serveButton.disableInteractive?.();}
        if(serveDom){serveDom.style.opacity='0';serveDom.style.transform='translate(-50%,-50%) scale(.92)';}
        if(order){order.style.opacity='.64';order.style.filter='saturate(.72)';}
      }else{
        if(serveDom){serveDom.style.opacity='1';serveDom.style.transform='translate(-50%,-50%) scale(1)';}
        if(order){order.style.opacity='1';order.style.filter='none';}
      }

      // A compact, soft halo follows the accepted target without boxing in the character illustration.
      if(focus){
        if(complete){focus.style.opacity='0';}
        else{
      ...
      soldBadge.textContent=complete?`완성 ${orders}/${total} · 주문 완료`:`완성 ${orders}/${total}`;soldBadge.style.background=complete?'rgba(239,250,244,.94)':orders?'rgba(255,244,248,.92)':'rgba(255,255,255,.78)';soldBadge.style.color=complete?'#39705a':'#79556d';if(orders>prevOrders){soldBadge.style.transform='scale(1.08)';setTimeout(()=>{if(soldBadge.isConnected)soldBadge.style.transform='scale(1)'},240);}prevOrders=orders;
      root.dataset.artQuality='authored-scene-v2';root.dataset.generatedVisualAssets='0';root.dataset.slimeColor=chosen.color||'';root.dataset.legacyJarVisible='0';root.dataset.shopVisible=String(shopVisible);root.dataset.finishedDisplay='completion-badge';root.dataset.completionCta=complete?'hidden':'active';root.dataset.roundComplete=String(complete);
    };
    scene.events.on('postupdate',sync);sync();
    if(window.__ADUGAME_ART_SOURCE__?.G3){window.__ADUGAME_ART_SOURCE__.G3.version='commercial-v2.3';window.__ADUGAME_ART_SOURCE__.G3.qualityTarget='G1R2 authored-art scene quality';window.__ADUGAME_ART_SOURCE__.G3.hero={name:'Baking with Mother',author:'oksmith',source:'Openclipart / publicdomainq.net',license:'Public Domain',url:BAKING};window.__ADUGAME_ART_SOURCE__.G3.legacyJarPresentation='hidden; completion badge used instead';window.__ADUGAME_ART_SOURCE__.G3.completionPresentation='submit CTA hidden after round complete';window.__ADUGAME_ART_SOURCE__.G3.generatedVisualAssets=0;}
    window.__ADUGAME_G3_COMMERCIAL_ART_V2__={loaded:true,version:'2.3',generatedVisualAssets:0,cohesiveOksmithScene:true,orderColorSynced:true,duplicateBowlRemoved:true,legacyJarHidden:true,compactFocus:true,completionCtaResolved:true};
