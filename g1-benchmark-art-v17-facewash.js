// ADUGAME G1R2 v17.12 face-wash scene.
...
(() => {
  ...
  function attach(scene){
    if(scene.scene?.key!=='G1R2'||scene.__g1v1712FaceWash)return;
    ...
    // Position the child's face over the real HabitV5 face-wash target (790,330).
    // The adult is intentionally pushed toward the upper edge so the child reads as the active subject.
    Object.assign(face.style,{position:'absolute',pointerEvents:'none',userSelect:'none',objectFit:'contain',left:'57%',top:'34%',width:'68%',height:'86%',transform:'translate(-50%,-50%)',zIndex:'3',display:'none'});
    root.appendChild(face);scene.__g1v1712FaceWash=face;root.dataset.faceWashReady='0';

    let loadedOnce=false;
    const loaded=()=>{
      if(loadedOnce)return;loadedOnce=true;
      root.dataset.faceWashReady='1';root.dataset.version='17.12';
      if(window.__ADUGAME_ART_SOURCE__?.G1R2){
        window.__ADUGAME_ART_SOURCE__.G1R2.version='v17.12';
      ...
      if(window.__ADUGAME_G1_BENCHMARK_ART_V17__)window.__ADUGAME_G1_BENCHMARK_ART_V17__.version='17.12';
    };
    ...
    const cleanup=()=>{face.remove();scene.__g1v1712FaceWash=null;if(people)people.style.visibility='visible';};
